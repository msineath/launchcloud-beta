...
export default function ProfilePage() {
  ...
  const userUploaded = songsArray.filter(
    (song) => song.uploaderId === Number(userId),
  );
  ...
  const selectedAlbumLikes = albumLikesArray.filter(
    (like) => like.userId === Number(userId),
  );
  const selectedAlbumLikeNames = selectedAlbumLikes
    .map((like) => albumsArray.filter((album) => like.albumId === album.id))
    .flat();

  const songLikes = useSelector((state) => state.songLikes);
  const songLikesArray = Object.values(songLikes);

  const selectedSongLikes = songLikesArray.filter(
    (like) => like.userId === Number(userId),
  );
  const selectedSongLikeNames = selectedSongLikes
    .map((like) => songsArray.filter((song) => like.songId === song.id))
    .flat();

  const albumComments = useSelector((state) => state.albumComments);
  const albumCommentsArray = Object.values(albumComments);
  const userAlbumComments = albumCommentsArray.filter(
  ...
  const userSongComments = songCommentsArray.filter(
    (comment) => comment.userId === Number(userId),
  );
      ...
      <div className='new-song-upload'>
        ...
        <form className='new-song-form' onSubmit={handleSubmit}>
          ...
          <input
            ...
            onChange={updateFile}
          />
          <button type='submit' className='new-info'>
            Add Song
          </button>
        </form>
      </div>
          ...
          {refinedAlbumComments ? (
            <>
              ...
              {refinedAlbumComments.map((albumId) => {
                const selected = albumsArray.find(
                  ...
                  <div className='profile-cell' key={albumId}>
                    <NavLink to={`/albums/${selected.id}`}>
                      <img
                        ...
                        alt='record-image'
                      />
                      <label className='album-choice'>{selected.name}</label>
                    </NavLink>
                  </div>
                );
              })}
                  ...
                  <div className='profile-cell' key={songId}>
                    <NavLink to={`/songs/${selected.id}`}>
                      <img
                        ...
                        alt='music-notes'
                      />
                      <label className='song-choice'>{selected.title}</label>
                    </NavLink>
                  </div>
                );
              })}
