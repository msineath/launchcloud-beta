...
export default function IndividualAlbumPage() {
  ...
  const selectedAlbum = albumsArray.filter(
  ...
  const selectedAlbumCredits = albumCreditsArray.filter(
    (credit) => credit.albumId === Number(albumId),
  );
  const allCreditNames = selectedAlbumCredits.map((credit) =>
  ...
  const selectedAlbumLikes = albumLikesArray.filter(
    (like) => like.albumId === Number(albumId),
  );
  const totalLikes = selectedAlbumLikes.filter(
    (like) => like.liked === true,
  ).length;
  const totalDisikes = selectedAlbumLikes.filter(
    (like) => like.disliked === true,
  ).length;

  const comments = useSelector((state) => state.albumComments);
  const commentsArray = Object.values(comments);
  const commentsOnAlbum = commentsArray.filter(
    (comment) => comment.albumId === Number(albumId),
  );
