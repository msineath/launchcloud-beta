...
export default function IndividualSongPage() {
  ...
  const selectedSongCredits = songCreditsArray.filter(
    (credit) => credit.songId === song.id,
  );
  const allCreditNames = selectedSongCredits.map((credit) =>
    Object.values(artistsArray).find((artist) => credit.artistId === artist.id),
  );

  const comments = useSelector((state) => state.songComments);
  const commentsArray = Object.values(comments);
  const commentsOnSong = commentsArray.filter(
    (comment) => comment.songId === Number(songId),
  );
  ...
  const filteredArray = likesArray.filter(
    (like) => like.songId === Number(songId),
  );
  const totalLikes = filteredArray.filter((like) => like.liked === true).length;
  const totalDisikes = filteredArray.filter(
    (like) => like.disliked === true,
  ).length;
  ...
  const creatorOptions = (buttonText, onClickFunction, newId) => {
    if (sessionUser?.id === song?.uploaderId) {
      return (
        <button id={newId} onClick={onClickFunction}>
          {buttonText}
        </button>
      );
    }
  ...
  const addComment = (event) => {
    ...
    if (commentText.length < 1) {
      return window.alert(
        'You must enter text to add a coment. Please Try again.',
      );
    }
  ...
  const editComment = (event) => {
    ...
    if (commentText.length === 0) {
      return window.alert(
        'Please add text to the textarea above the edit comment button first, then click edit comment.',
      );
    }
    ...
    <div className='frame'>
      ...
      <div className='song-data-parent'>
        <div className='individual-song-info'>
          <label className='data-tag'>Title:</label>
          <label>
            {song?.title}
            {creatorOptions(
              'Edit',
          ...
          <div className='genre-info'>
            <label className='data-tag'>Genre:</label>
            <label>
              {song?.genre}
              {creatorOptions(
                'Edit',
          ...
          <div className='release-date-info'>
            <label className='data-tag'>Release Date:</label>
            <label>
              {date}
              {creatorOptions(
                'Edit',
      ...
      <div className='song-comments'>
        ...
        <form onSubmit={addComment} className='song-comment-box'>
          <textarea
