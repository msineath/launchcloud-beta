...
export default function IndividualArtistPage() {
  ...
  const artistAlbumCredits = albumCreditsArray.filter(
    (credit) => credit.artistId === Number(artistId),
  );

  const albums = useSelector((state) => state.albums);
  const albumsArray = Object.values(albums);
  const artistAlbums = artistAlbumCredits
    .map((credit) => albumsArray.filter((album) => credit.albumId === album.id))
    .flat();
  ...
  const artistSongCredits = songCreditsArray.filter(
    (credit) => credit.artistId === Number(artistId),
  );
  ...
  const artistSongs = artistSongCredits
    .map((credit) => songsArray.filter((song) => credit.songId === song.id))
    .flat();
